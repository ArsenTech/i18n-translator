            ...
            Event::Start(e) if inside_data && e.name().as_ref() == b"value" => {
                ...
                if let Some(key) = &current_key {
                    if let Some(value) = lookup.get(key.as_str()) {
                        writer
                            .write_event(Event::Text(BytesText::from_escaped(value.to_string())))
                            .map_err(|e| e.to_string())?;
                    }
                }
            }
                ...
                if !saw_value {
                    ...
                    writer
                        .write_event(Event::Text(BytesText::from_escaped(
                            lookup
                                .get(current_key.as_deref().unwrap_or(""))
                                .unwrap_or(&"")
                                .to_string(),
                        )))
                        .map_err(|e| e.to_string())?;
...
pub fn open(base_path: String, target_path: String) -> Result<Vec<TranslationEntry>, String> {
    open_xml_with_callback(base_path, target_path, |c| parse_resx(c))
}
pub fn save(target_path: String, entries: Vec<TranslationEntry>) -> Result<(), String> {
    save_xml_with_callback(target_path, entries, None, None, |o| {
        upsert_resx(&o.content, &o.entries)
    })
}
