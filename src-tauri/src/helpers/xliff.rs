use crate::{
    helpers::save_xml_with_callback,
    types::structs::{CreateTranslationResult, ParsedXliff, TranslationEntry, XliffMetadata},
};
use quick_xml::{
...
fn upsert_xliff(
    ...
    let lookup: HashMap<&str, &TranslationEntry> =
        entries.iter().map(|e| (e.key_name.as_str(), e)).collect();
    ...
    loop {
            ...
            .map_err(|e| e.to_string())?
        {
            Event::Start(e) if e.name().as_ref() == b"xliff" => {
                ...
                for attr in e.attributes() {
                    ...
                    match attr.key.as_ref() {
                        ...
                        _ => {
                            xliff.push_attribute((attr.key.as_ref(), attr.value.as_ref()));
                        }
                    }
                }
            ...
            Event::End(e) if inside_segment && e.name().as_ref() == b"segment" => {
                if !saw_target {
                    let value = current_key
                        .as_deref()
                        .and_then(|key| lookup.get(key))
                        .map(|entry| entry.translation_string.as_str())
                        .unwrap_or("");
                    writer
            ...
            Event::End(e) if e.name().as_ref() == b"file" => {
                for entry in entries {
                    ...
                    writer
                        ...
                        .map_err(|e| e.to_string())?;
                    writer
                        .write_event(Event::Text(BytesText::from_escaped(&entry.base_string)))
                        .map_err(|e| e.to_string())?;
                    writer
...
pub fn create(
    ...
    let entries: Vec<TranslationEntry> = parsed
        .entries
        .into_iter()
        .enumerate()
    ...
    fs::write(
        &target_file,
        upsert_xliff(&content, &entries, &base_lang, &target_lang)?,
    )
    .map_err(|e| e.to_string())?;
