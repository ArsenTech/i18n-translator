    ...
    loop {
            ...
            .map_err(|e| e.to_string())?
        {
            ...
            Event::Text(_) | Event::GeneralRef(_) | Event::CData(_)
                if inside_string && replacing_string => {}
            ...
            Event::End(e) if e.name().as_ref() == b"resources" => {
                for entry in entries {
                    ...
                    writer
                        ...
                        .map_err(|e| e.to_string())?;
                    writer
                        .write_event(Event::Start(string))
                        .map_err(|e| e.to_string())?;
                    writer
                        .write_event(Event::Text(BytesText::from_escaped(
...
pub fn create(
    ...
    let res_dir = values_dir
        .parent()
        .ok_or("Invalid Android resources path")?;
...
pub fn open(base_path: String, target_path: String) -> Result<Vec<TranslationEntry>, String> {
    open_xml_with_callback(base_path, target_path, |c| parse_android_xml(c))
}
pub fn save(target_path: String, entries: Vec<TranslationEntry>) -> Result<(), String> {
    save_xml_with_callback(target_path, entries, None, None, |o| {
        upsert_android_xml(&o.content, &o.entries)
    })
}
